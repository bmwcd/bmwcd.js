...
(async () => {
  ...
  status = await vehicle.status(true, true)
  const dataPath = path.join(process.cwd(), 'data', moment().format('MM-YYYY'), vehicle.vin.substr(-7, 7))
  const dataFile = path.join(dataPath, moment().format('[status_min_]X[.json]'))
