#!/usr/bin/env node
/*
            bimmer.js : BMW ConnectedDrive module for Node
---------------------------------------------------------------------------
    dMMMMb  dMP dMMMMMMMMb dMMMMMMMMb dMMMMMP dMMMMb       dMMMMMP .dMMMb
   dMP"dMP amr dMP"dMP"dMPdMP"dMP"dMPdMP     dMP.dMP          dMP dMP" VP
  dMMMMK" dMP dMP dMP dMPdMP dMP dMPdMMMP   dMMMMK"          dMP  VMMMb
 dMP.aMF dMP dMP dMP dMPdMP dMP dMPdMP     dMP"AMF  amr dK .dMP dP .dMP
dMMMMP" dMP dMP dMP dMPdMP dMP dMPdMMMMMP dMP dMP  dMP  VMMMP"  VMMMP"
--------------------------------------------------------------------------
  Maintained by Nicholas Berlette  <https://github.com/bmwcd/bimmer.js>
--------------------------------------------------------------------------
*/
...
let tokenExpiration, accessToken
const BMW_TOKEN_FILE = './token.json'
const BMW_TOKEN_MINIMUM = (10 * 60) // 10 minutes * 60 seconds
...
module.exports = {
  ...
  async parseVehicle (data = {}) {
    ...
    const parsedVehicle = {
      ...
      options: {
        ...
        // "dealer": {
        //     "name": "BMW of Las Vegas",
        //     "street": "6900 W Sahara Ave",
        //     "postalCode": "89117",
        //     "city": "Las Vegas",
        //     "country": "US",
        //     "phone": "001 702 5700279"
        // },
        // "breakdownNumber": "",
        // "countryCode": "V2-US",
        // "egoVehiclePath": "",
  ...
  async parseLocation (status = {}) {
    ...
    const parsedLocation = {
      string: `${Number(status.attributesMap.gps_lat)}, ${Number(status.attributesMap.gps_lng)}`,
  ...
  async auth (username, password) {
    ...
    _log('Authenticating...')
    let token = require(BMW_TOKEN_FILE)
    if (moment(token.expiration).valueOf() <= moment().valueOf() + (BMW_TOKEN_MINIMUM * 1000)) {
      _log('Requesting new token, please wait...')
      try {
        ...
        tokenExpiration = moment(moment().valueOf() + (1000 * tokenData.expires_in), false)
      } catch (err) {
        _error(err)
      ...
      fs.writeFileSync(BMW_TOKEN_FILE, JSON.stringify(token, null, 2), 'utf8')
    } else {
      _log(`Old token still valid: ${token.accessToken}`)
      ...
      async findVehicle (vehicleVin) {
        ...
        return {
          ...
          async status (parseData = true) {
            ...
            _log(`Requesting status for ${vehicle.vin}...`)
            try {
              [error, result] = await to(
                request.get(util.format(BMW_URLS.status, vehicle.vin), { headers: headers })
              )
              if (error) _error(error)
              else return parseData === true ? bimmerjs.parse(vehicle, result.data) : result.data
            } catch (err) {
              throw _error(err)
            }
          }, // status()
          ...
          async location () {
            return bimmerjs.parseLocation(await this.status(false))
          }, // location()
          ...
          async remoteService (serviceType = BMW_SERVICES.DOOR_LOCK) {
            if (vehicle === undefined) return _error('Invalid vehicle')
            _log(`Requesting BMW_SERVICES.${serviceType} for ${vehicle.vin}...`)
            try {
              [error, result] = await to(
                request.post(
          ...
          // Remote Service Status functions

          async remoteServiceStatus (serviceType = BMW_SERVICES.DOOR_LOCK) {
            if (vehicle === undefined) return _error('Invalid vehicle')
            _log(`Retrieving lock request status from ${vehicle.vin}...`)
            try {
              [error, result] = await to(
                request.get(
