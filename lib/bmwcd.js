...
const KeyMap = {
  ...
  lsc: {
    ...
    DOORSTATECHANGE: 'State Change: Doors',
    LOCKSTATECHANGE: 'State Change: Locks'
  }
}
...
module.exports = {
  ...
  async parseStatus (vin, status = {}) {
    ...
    const parsedStatus = {
      ...
      info: {
        vin: vin,
        mileage: status.attributesMap.mileage,
        updateReason: KeyMap.lsc[status.attributesMap.lsc_trigger],
        lscTrigger: status.attributesMap.lsc_trigger,
        parkingLights: KeyMap.lights[status.attributesMap.lights_parking],
      ...
      time: {
        ...
        friendly: updateTime.format('dddd [at] h:mm a')
      },
      location: {
        ...await this.parseLocation(status)
      },
      economy: {
        rangeKm: status.attributesMap.beRemainingRangeFuelKm,
        rangeMiles: status.attributesMap.beRemainingRangeFuelMile,
        fuelLiters: status.attributesMap.remaining_fuel,
        fuelGallons: cnv.toGallons(status.attributesMap.remaining_fuel)
      },
      lids: {
        locked: status.attributesMap.door_lock_state,
    ...
    const parsedMessages = {
      ...
      cbsData: [
        ...status.vehicleMessages.cbsMessages.map(cbs => {
          const cbsMessage = {
            type: cbs.text.toUpperCase().replace(' ', '_'),
            description: cbs.description,
            dueDate: moment(cbs.date).format('ddd MMM Do YYYY'),
            fromNow: moment(cbs.date).fromNow(true)
          }
          if ([null, undefined].includes(cbs.unitOfLengthRemaining) === false) {
            let mileage = status.attributesMap.mileage // miles
            let distanceLeft = cbs.unitOfLengthRemaining // km
            if (/^mls?|miles?$/i.test(status.attributesMap.unitOfLength)) distanceLeft = cnv.toMiles(distanceLeft, 0) // km -> miles
            else mileage = cnv.toKilom(mileage, 0) // miles -> km
            cbsMessage.distanceLeft = Math.round(distanceLeft / 100) * 100
            cbsMessage.distanceDue = Math.round((distanceLeft + Math.round(mileage)) / 100) * 100
          }
          return cbsMessage
        })
      ]
  ...
  async auth (username, password) {
    ...
    if (toker.check(token)) {
      ...
      _log(` >> Token expires ${moment(token[0]).fromNow()}`)
    } else {
